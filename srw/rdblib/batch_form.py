...
class BatchForm(object):
    ...
    def _set_deletion_state(self, set_as_deleted):
        ...
        if set_as_deleted:
            ...
            marker = tiff_handler.long_data2.rec.page_name

        # First, change the data of the form header.
        cdb_form.form_header.update_rec(imprint_line_long=marker, imprint_line_short=marker)
        ...
        tiff_handler.long_data.update_rec(page_name=marker)
        # This line also patches the new data into the tiff structure (in memory).
        ibf_data.update_rec(codnr=marker)
