...
def walther_tags(*, width, height, page_name, dpi=200, dt=None, extra_tags=None):
    if dt is None:
        dt = DateTime.now()
    tags = OrderedDict([
        (TT.NewSubfileType, 0), # "general indication of the kind of data contained in this subfile", default = 0
        (TT.ImageWidth,     width),
        (TT.ImageLength,    height),
        (TT.BitsPerSample,  1),
        (TT.Compression,    4),
        (TT.PhotometricInterpretation, 0), # 0 = "schwarz auf weiß"
        (TT.FillOrder,              1), # 1 = "von oben links nach unten rechts"
        ...
        (TT.Orientation,            1),
        (TT.MinSampleValue,         0), # "minimum component value used", TIFF default = 0
        (TT.MaxSampleValue,         1), # "maximum component value used", TIFF default = 2**(BitsPerSample) - 1
        (TT.SamplesPerPixel,        1), # 1 = Graustufen
